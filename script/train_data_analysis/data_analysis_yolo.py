...
def class_and_num(root, class_all_name):
    """
    计算每个类别的目标个数
    """
    print('计算每个目标的个数')
    ...
    with open(class_all_name, 'r') as f:
        names = np.array([x.split() for x in f.read().strip().splitlines()], dtype=np.str)  # labels
    a = np.zeros((len(names)), dtype=np.int)
    txt_file_se = "labels_yolo/*.txt"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--root', type=str,
                        default="/home/lishuang/Disk/shengshi_data/jiaotong/",
                        help='root dir')
    opt = parser.parse_args()

    root = opt.root
    num_list = class_and_num(root, os.path.join(root,'classes.names'))
